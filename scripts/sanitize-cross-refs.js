...
// Sanitize arrays in every command file
fileMap.forEach((full, slug) => {
  ...
  rel = Array.from(new Set(rel)).filter((s) => s !== slug);
  alt = Array.from(new Set(alt)).filter((s) => s !== slug);
...
// Synchronize bidirectional links
...
console.log('Sanitized and synchronized cross-references successfully across all commands.');
